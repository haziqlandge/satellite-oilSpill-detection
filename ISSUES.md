...
| F4 | **The particle renderer is still crude** — square `fillRect` parcels, linear interpolation between stored frames, `clearRect` every frame, no per-particle variation. The two causes that made playback a slideshow are fixed: the canvas now follows a fractional playhead, and the backward leg is the ensemble's own frames rather than a scaled copy of T0. What is left is draw quality. `FUTURE_WORK.md` §1.4. |
