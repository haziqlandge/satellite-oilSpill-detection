...
| Q5 | **The largest full-scene detections are whole-tile masks, and the real drift runs are seeded from them.** In all three processed Gulf scenes the biggest polygons have a bounding box of 0.94 × 0.94 of a 1024-px inference tile: the model marked every valid pixel of the window and found no slick edge inside it, and SNAP's land-sea mask then cut the block to the shore. May's eight largest are all such blocks, several wholly at sea at confidence .25–.32 (wind at the pass 2.6 m/s, below the ~3 m/s Bragg gate — the whole sea is dark). Each real run's seed is one: April's fills 92% of its tile (30% of it on GSHHG land); December's lies **100% on GSHHG land**, an inland water body, and OpenDrift had to move its parcels off (T0 spread 6.7 km against 0.6 km for the others). Input preparation was checked and matches training (band 2, zeros → 0 dB → white in both `db_to_uint8` paths, mask ANDed with valid pixels), so this is the model's look-alike behaviour (Q2), not a preparation bug. Measured 2026-09-23. The seed rule ("largest detection") is waiting on the user — see `FUTURE_WORK.md` §0. | `eval/final/scenes/*.geojson`; `frontDemo/public/runs/*/scene.json` |
