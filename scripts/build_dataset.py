...
* **Single-class.** The corpus is binary oil/not-oil. `oos` versus
  ...
  `--classes two` requires a confirmed-review file and refuses without it.
"""
...
def _collect(sensors: tuple[str, ...] | None) -> list[SourceImage]:
    ...
    if part_three.is_dir():
        ...
        print(f"  Part III      {len(found):>6} images ({sum(s.is_negative for s in found)} negatives)")
        sources.extend(found)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="JSON of {identity: {instance: class}} from the relabelling review",
    )
    ...
    if arguments.classes == "two":
        if arguments.confirmed is None:
            ...
            return 1
        confirmed = json.loads(arguments.confirmed.read_text(encoding="utf-8"))
        confirmed = {k: {int(i): c for i, c in v.items()} for k, v in confirmed.items()}
    ...
    try:
        report = build_dataset(
            sources, arguments.out, class_names=class_names, confirmed=confirmed
        )
