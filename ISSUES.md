...
| X3 | **Only an ERA5 wind fetcher exists; GFS and CMEMS have none.** `backend/ingest/metocean/era5.py` (`fetch_era5_wind`, `era5_reader`) plugs into the caching layer in `cache.py` and forced the three real runs. Currents have no fetcher, and could not be fetched anyway without CMEMS credentials (X2), so every real run is wind-only. |
