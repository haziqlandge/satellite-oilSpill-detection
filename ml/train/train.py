...
def allowed_cores(
    fraction: float = CPU_CEILING_FRACTION, *, prefer_efficiency: bool = True
) -> list[int]:
...
class TrainingTimeBudget:
    ...
    def on_fit_epoch_end(self, trainer: Any) -> None:
        if self.started_at is None:
            return
        if getattr(trainer, "epoch", -1) + 1 >= getattr(trainer, "epochs", float("inf")):
            return  # Let final validation/finalization complete instead of pausing at the last epoch.
...
def train(
    ...
    *,
    ...
    results = trained.train(**train_kwargs(config, project=project, seed=seed, overrides=overrides))
