...
so the pair is swapped exactly once, here. Both grid-unit box tensors are
converted to pixels with the supplied per-anchor stride before MPDIoU.
"""
...
def _build_class() -> type:
    ...
    class _MPDIoUBboxLoss(BboxLoss):  # type: ignore[misc, valid-type]
        ...
        def forward(
            ...
            # Ultralytics supplies grid-unit boxes and one pixel stride per anchor.
            # Broadcast over images BEFORE foreground indexing: selected anchors
            # may come from different feature levels in every image.
            prediction_pixels = (pred_bboxes * stride)[fg_mask]
            target_pixels = (target_bboxes * stride)[fg_mask]
            similarity = mpdiou(prediction_pixels, target_pixels, image_size=size, xywh=False)
