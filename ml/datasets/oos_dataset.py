...
def mask_to_polygons(
    ...
    *,
    ...
    """Return polygons while preserving the historical single-class geometry."""
    return [
        polygon
        for _, polygon in _labelled_polygons(
            mask, min_area_px=min_area_px, tolerance_px=tolerance_px
        )
    ]
...
def _labelled_polygons(
    ...
    *,
    ...
    polygons: list[tuple[int, tuple[tuple[float, float], ...]]] = []
    for index in range(1, int(labelled.max()) + 1):
        ...
        if len(points) >= MIN_POLYGON_POINTS:
            polygons.append((index, tuple(points)))
...
def instances_for(
    ...
    *,
    ...
    if confirmed is None and tuple(class_names) != (SINGLE_CLASS_NAME,):
        raise DatasetError("multi-class instances require confirmed review labels")
    polygons = _labelled_polygons(mask, min_area_px=min_area_px)
    instances: list[Instance] = []

    for order, polygon in polygons:
        if confirmed is None:
            ...
            if name not in class_names:
                raise DatasetError(
                    f"confirmed class {name!r} is not in the scheme {tuple(class_names)}"
                )
...
def db_to_uint8(array: np.ndarray, *, window: tuple[float, float] = DB_WINDOW) -> np.ndarray:
...
def build_dataset(
    ...
    *,
    ...
    if tuple(class_names) != (SINGLE_CLASS_NAME,) and root.exists():
        raise DatasetError(
            "multi-class builds require a new output directory; stale labels must not survive review exclusions"
        )
