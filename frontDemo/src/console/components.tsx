  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2
          className="text-[11.5px] tracking-[0.3em] uppercase"
 ...
 *
 * The heading runs the full width of the pane in both layouts. A margin-column
 * version of this was built and thrown away: it turned every label ninety
 * degrees and switched off the rule, which is the one piece of furniture that
 * says a console pane is a console pane. The width is worth using inside the
 * body, where a figure can sit beside its explanation -- see `Split` -- and not
 * by relocating the labels.
 *
 * `data-block` and `data-block-body` are handles for the panel reader's scoped
 * CSS. Same precedent as `data-pane-body`: one component serving two layouts
 * rather than a fork.
 */
...
/**
 * A figure beside the text that explains it.
 *
 * In a dock rail there is no room for this and it renders as what it already
 * was: the figure, then the rows and the note under it. In the panel reader
 * there is room, and stacking them there wastes it twice over -- the page ends
 * up as one tall column with several hundred pixels of nothing to the right of
 * every paragraph, and the reader has to hold a plot in mind while scrolling
 * past the sentences about it.
 *
 * Only used where there is genuinely both. A block that is all readings stays
 * one full-width column, and so does a block whose figure has nothing to say
 * beside it; two columns is a shape for a pairing, not a house style.
 *
 * The DOM order is figure first, which is the order a rail already had. The
 * reader puts the text on the left with explicit grid columns rather than by
 * reordering, so a rail keeps exactly the sequence it has today.
 */
export function Split({
  ...
  return (
    <div data-block-split>
      <div data-split-figure>{figure}</div>
      <div data-split-text>{children}</div>
    </div>
  );
}
...
/**
 * The band that names a group of controls.
 *
 * The console's labels were all one weight -- 9px, letterspaced, `--ink-faint`
 * -- whether they introduced a group of live switches or a static readout.
 * Nothing announced itself as a place where things could be changed, so a
 * viewer had to discover the layer switches by trying them.
 *
 * This is the fix, and it is deliberately the loudest label in the family: a
 * filled band in a dark green with light ink on it, so a group of controls
 * reads as a control surface from across the room. Used for CONTROL ATTRIBUTES
 * over the layer switches and for the other operable groups, never over a
 * group that only reports.
 */
    ...
    <div className="mt-3 first:mt-0">
      ...
      >
        <h3
          ...
          style={{ color: "var(--group-ink)", fontWeight: 500 }}
        >
          {children}
        </h3>
      ...
      {hint && (
        <p
          className="mt-1.5 px-2 text-[10.5px] leading-[1.6]"
          style={{ color: "var(--ink-faint)" }}
        >
          {hint}
        </p>
