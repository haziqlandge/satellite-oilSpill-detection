...
export interface MapPaint {
   ...
   *
   * Esri's rasters are neutral grey, and `raster-saturation` cannot put colour
   * into a source that has none -- there is no chroma there to rotate. A
   * direction that wants the world in its own ink therefore tints it from
   * above, with a translucent background layer between the raster and the data
   * layers. Omit it and the world stays grey.
   */
  basemapTint?: string;
...
export const DESIGNS: DesignDef[] = [
...

  {
    ...
    map: {
      water: "#050706",
      // A console with no world under it made the graticule carry all the
      // geography, and a degree grid alone cannot say where the coast is. The
      // dark grey canvas is the least decorative world available -- coastline
      // and almost nothing else -- held far back and washed in the direction's
      // own green so it reads as part of the console rather than as a map
      // borrowed from somewhere brighter.
      basemap: "canvas",
      basemapOpacity: 0.85,
      basemapSaturation: -1,
      // Contrast left at zero and the dynamic range clamped hard. The dark grey
      // canvas is already a near-black raster; adding contrast crushes it to
      // solid black and the world disappears entirely, which is how a basemap
      // ends up loading correctly and rendering as nothing.
      basemapContrast: 0,
      basemapBrightnessMax: 0.28,
      basemapTint: "#04150c",
      basemapTintOpacity: 0.4,
      showLabels: false,
      slick: "#4fe08a",
      slickUnknown: "#4d6a5c",
      contour50: "#8bffc4",
...

  {
    ...
    map: {
      ...
      traffic: "#212c36",
      graticule: "#2a3945",
      // The one direction whose ground is genuinely blue. A grey-blue candidate
      // track sat inside the bathymetry rather than on it and was unfindable
      // before it was ever compared to anything; orange is the far side of the
      // wheel from both the chart and the cyan selection, so a gated track
      // separates from the sea by hue and from the selected one by weight.
      candidate: "#ff8a3d",
      suspect: "#4fd8e8",
      target: "#e2ecf5",
      dark: "#ff7a7a",
