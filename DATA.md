...
| `frontDemo/public/runs/<scene>/scene.json` | 68-606 KB | Beside each `drift.json`: the model's full-scene detections (simplified to 0.0002°, seed flagged) and the ERA5 wind at the seed from the drift's own cached request. `python -m scripts.export_real_scenes` (offline; reads the ERA5 cache). The flagged seed is the detection `choose_seed` picked (largest at sea without a box-cut edge; `ISSUES.md` Q5). **Gitignored** with the rest of `public/runs/`, like `drift.json`: regenerate on any other machine, after `export_drift_runs` |
