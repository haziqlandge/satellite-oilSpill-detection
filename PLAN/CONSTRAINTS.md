...
| C12 | **The demo must degrade gracefully without network** | Amended from "runs fully offline". The database is hosted, so a snapshot export plus a `DEMO_OFFLINE=1` read path is now the mechanism (PHASE-09). Venue wifi must not be a single point of failure. **Amended 2026-09-25 (user decision): the demo is always online.** It is the Vercel site plus a local API, so a network-off run is no longer a PHASE-09 acceptance line. The degrade path stays and is tested by its guard (`scripts/verify_offline.py`, 0 reaches over the whole live pipeline): cache-first forcing, `DEMO_OFFLINE=1`, and land from the local mask when tiles fail. **Supabase is not used.** The API is file-backed (ISSUES X12), the project stays paused, and later online work is planned on Cloudflare. The local snapshot (`scripts/export_snapshot.py`) replaces PHASE-09's "pre-seeded in Supabase" | Discovering on demo day that nothing loads |
...
## Current machine authorization — 2026-09-15

The user is on RTX 4060 Ti **8 GB**, 32 GB RAM and explicitly permits full CPU/GPU/RAM use. The 2026-08-31 laptop caps below are historical. See [screening review](../eval/screening/REVIEW.md) for data/loss integrity findings; scientific constraints C1–C12 remain in force.

## Historical laptop limits (2026-08-31; superseded on current machine)
