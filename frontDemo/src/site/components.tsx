 ...
 *
 * There used to be two widths on this page and they did not agree: text sat on
 * an 1180px grid behind an 80px gutter, so it began around x=367 on a wide
 * screen, while figures sat on a 1400px grid beginning at x=177. Figures were
 * therefore both wider than the prose *and* further left than it, which read
 * exactly as it was -- pictures sprawling while the writing was squeezed into a
 * narrower column set in from them.
 *
 * One frame now. Prose, figures, maps and the masthead all share this width and
 * this left edge, and nothing on the page is allowed a second one.
 */
export function Page({ children }: { children: ReactNode }) {
  return <div className="mx-auto w-full max-w-[1120px] px-5 sm:px-8 lg:px-10">{children}</div>;
}
 ...
 *
 * The third column is **bounded and the grid is centred**, and that is the whole
 * point of this rule. It used to be `minmax(0, 1fr)` inside a 1560px page, which
 * measured 674px at a 1754px viewport while the margin note inside it measured
 * 236px -- 438px of dead space beside every prose block on the page, which read
 * as the text floating in a void rather than as a composition.
 *
 * A text grid is measured; figures may be wider. `Page` stays wide for the
 * figures, and only this grid is held to a measure.
 */
export function Spread({
  ...
  return (
    <div
      className={`grid grid-cols-1 gap-x-10 gap-y-6 lg:grid-cols-[minmax(0,60ch)_minmax(0,1fr)] ${className}`}
    >
      {children}
    </div>
  );
 ...
 * A figure row.
 *
 * Full frame width, deliberately identical to the frame the prose is set on --
 * it exists to mark intent at the call site, not to introduce a second width.
 * It used to carry its own `max-w-[1400px]`, which is precisely how figures
 * ended up wider than the writing they belonged to.
 */
...
/* The spread has two columns now, not three. The empty 5rem gutter was what
   pushed every paragraph in from the left edge the figures were using. */
...
export function Measure({
  ...
  return <div className={`lg:col-start-1 ${className}`}>{children}</div>;
}

/**
 * The marginal column.
 *
 * No inner width cap. It used to carry `lg:max-w-[26ch]`, which was the second
 * half of the dead-space bug: the column was already stretching to fill, and
 * then its content refused to. The column is bounded at the grid now, so the
 * content simply fills it.
 */
export function Margin({ children }: { children?: ReactNode }) {
  return <div className="lg:col-start-2 lg:pt-1.5">{children}</div>;
}
