...
def run_cell(
    ...
    *,
    ...
    if cell.loss == "mpdiou":
        # The criterion is built lazily, so the swap has to happen after the
        # first loss call. `on_train_start` is too early; `on_train_batch_end`
        # of the first batch is the earliest point the object exists.
        state = {"swapped": 0}
        ...
        model.add_callback("on_train_batch_end", _swap)

    install_gpu_usage_limit(model)
    install_training_time_budget(model, max_minutes)
    kwargs = train_kwargs(config, project=project, seed=seed)
    if resuming:
...
def write_results(project: Path, epochs: int, destination: Path = RESULTS) -> Path:
    ...
    destination.write_text(
        ...
        "`workers` deviates from the paper's 8 to keep machine-wide RAM under a "
        "24 GB ceiling; it affects wall-clock only, not results.\n\n"
